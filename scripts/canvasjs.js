...
(function () {
	...
	var numberFormat = function (v, fs, cultureInfo) {
		...
		var digitGroupSeparator = cultureInfo
		...
		result +=
			(addDecimalSeparator ? decimalSeparator : '') + resultAfterDecimal;
	...
	var devicePixelBackingStoreRatio = optimizeForHiDPI
	...
	CanvasJSObject.prototype.setOptions = function (
		options,
		currentThemeOptions
	) {
		if (!defaultOptions[this._defaultsKey]) {
			...
			for (var prop in defaults) {
				if (options && prop in options) this[prop] = options[prop];
	...
	CanvasJSObject.prototype.updateOption = function (prop) {
		...
		if (prop in defaults) {
			if (prop in this._options) newValue = this._options[prop];
			else if (currentThemeOptions && prop in currentThemeOptions)
	...
	CanvasJSObject.prototype.isBeingTracked = function (option) {
		...
		if (this._options._oldOptions[option]) return true;
		else return false;
	};
	...
	CanvasJSObject.prototype.addEventListener = function (
		eventName,
		eventHandler,
		context
	) {
	...
	CanvasJSObject.prototype.removeEventListener = function (
		eventName,
		eventHandler
	) {
	...
	CanvasJSObject.prototype.dispatchEvent = function (
		eventName,
		eventParameter
	) {
		if (!eventName || !this._eventListeners[eventName]) return;
	...
	function Chart(containerId, options, publicChartReference) {
		...
		Chart.base.constructor.call(
			this,
			'Chart',
			options,
			options.theme ? options.theme : 'theme1'
		);
		...
		this._container =
			typeof this._containerId === 'string'
		...
		if (this._options.width) width = this.width;
		else
			width =
		...
		if (this._options.height) height = this.height;
		else
			height =
		...
		this._selectedColorSet =
			typeof colorSets[this.colorSet] !== 'undefined'
				? colorSets[this.colorSet]
				: colorSets['colorSet1'];
	...
	Chart.prototype._updateOptions = function () {
		...
		if (this._options.zoomEnabled) {
			...
			if (!this._resetButton) {
				...
				addEvent(this._resetButton, 'click', function () {
					...
					if (_this._options.axisX && _this._options.axisX.minimum)
						_this.sessionVariables.axisX.internalMinimum =
							_this._options.axisX.minimum;
					else _this.sessionVariables.axisX.internalMinimum = null;

					if (_this._options.axisX && _this._options.axisX.maximum)
						_this.sessionVariables.axisX.internalMaximum =
							_this._options.axisX.maximum;
					else _this.sessionVariables.axisX.internalMaximum = null;
			...
			if (!this.zoomEnabled && !this.panEnabled) {
				if (!this._zoomButton) {
					...
					if (
						_this._zoomButton.getAttribute('state') ===
						_this._cultureInfo.zoomText
					) {
		...
		if (this._menuButton) {
			if (this.exportEnabled) show(this._menuButton);
			else hide(this._menuButton);
			...
			addEvent(
				...
				function () {
					if (_this._dropdownMenu.style.display === 'none') {
						if (
							_this._dropDownCloseTime &&
							new Date().getTime() - _this._dropDownCloseTime.getTime() <= 500
						)
							return;
		...
		if (this._toolBar.style.display !== 'none' && this._zoomButton) {
			...
			if (
				_this._resetButton.getAttribute('state') !==
				_this._cultureInfo.resetText
			)
	...
	Chart.prototype._updateSize = function () {
		...
		if (
			this.canvas.width !== width * devicePixelBackingStoreRatio ||
	...
	Chart.prototype._initialize = function () {
		...
		if (!this._animator) this._animator = new Animator(this);
		else {
		...
		this.animatedRender =
			isCanvasSupported && this.animationEnabled && this.renderCount === 0;
		...
		for (var series = 0; series < this._options.data.length; series++) {
			...
			if (
				!(
					!this._options.data[series].type ||
					Chart._supportedChartTypes.indexOf(this._options.data[series].type) >=
						0
				)
			)
				continue;
			...
			if (dataSeries.color === null) {
				if (this._options.data.length > 1) {
					dataSeries._colorSet = [
						this._selectedColorSet[
							dataSeries.index % this._selectedColorSet.length
						]
					];
					dataSeries.color = this._selectedColorSet[
						dataSeries.index % this._selectedColorSet.length
					];
				} else {
					if (
						...
						dataSeries.type === 'stepArea' ||
						dataSeries.type === 'splineArea' ||
						dataSeries.type === 'stackedArea' ||
						dataSeries.type === 'stackedArea100' ||
						dataSeries.type === 'rangeArea' ||
						dataSeries.type === 'rangeSplineArea' ||
			...
			if (dataSeries.markerSize === null) {
				if (
					((dataSeries.type === 'line' ||
						...
						dataSeries.dataPoints.length < this.width / 16) ||
			...
			if (
				(dataSeries.type === 'bubble' || dataSeries.type === 'scatter') &&
				dataSeries.dataPoints
			) {
			...
			if (seriesAxisPlacement === 'normal') {
				if (this.plotInfo.axisPlacement === 'xySwapped') {
					errorMessage =
						'You cannot combine "' + dataSeries.type + '" with bar chart';
				} else if (this.plotInfo.axisPlacement === 'none') {
					errorMessage =
						'You cannot combine "' + dataSeries.type + '" with pie chart';
	...
	Chart._supportedChartTypes = addArrayIndexOf([
		'line',
		'stepLine',
		'spline',
		'column',
	...
	Chart.prototype.render = function (options) {
		...
		if (this._options.title) {
			...
			if (!this._title.dockInsidePlotArea) this._title.render();
			else plotAreaElements.push(this._title);
		}
		...
		if (this._options.subtitles) {
			for (var i = 0; i < this._options.subtitles.length; i++) {
				...
				if (!subtitle.dockInsidePlotArea) subtitle.render();
				else plotAreaElements.push(subtitle);
			}
		}
		...
		for (var i = 0; i < this.data.length; i++) {
			if (
				...
				this.data[i].type === 'pie' ||
				this.data[i].type === 'doughnut'
			) {
				this.legend.dataSeries.push(this.data[i]);
			}
		}

		if (!this.legend.dockInsidePlotArea) this.legend.render();
		else plotAreaElements.push(this.legend);
		...
		for (var i = 0; i < this.plotInfo.plotTypes.length; i++) {
			...
			for (var j = 0; j < plotType.plotUnits.length; j++) {
				...
				else if (plotUnit.type === 'pie') this.renderPie(plotUnit);
				else if (plotUnit.type === 'doughnut') this.renderPie(plotUnit);
				...
				for (var k = 0; k < plotUnit.dataSeriesIndexes.length; k++) {
					this._dataInRenderedOrder.push(
						this.data[plotUnit.dataSeriesIndexes[k]]
					);
				}
			...
			_this._animator.animate(
				...
				function (fractionComplete) {
					...
					_this.ctx.drawImage(
						...
						0,
						0,
						Math.floor(_this.width * devicePixelBackingStoreRatio),
					...
					for (var l = 0; l < animations.length; l++) {
						...
						if (
							fractionComplete < 1 &&
							typeof animationInfo.startTimePercent !== 'undefined'
						) {
							if (fractionComplete >= animationInfo.startTimePercent) {
								...
								animationInfo.animationCallback(
									animationInfo.easingFunction(
										fractionComplete - animationInfo.startTimePercent,
										0,
										1,
										1 - animationInfo.startTimePercent
									),
									animationInfo
								);
							}
							...
							animationInfo.animationCallback(
								animationInfo.easingFunction(fractionComplete, 0, 1, 1),
								animationInfo
							);
						}
					}
			...
			if (Chart._supportedChartTypes.indexOf(dataSeries.type) >= 0) {
				...
				if (!plotUnitExists) {
					plotUnit = {
						...
						axisY:
							dataSeries.axisYType === 'primary' ? this.axisY : this.axisY2,
						axisX: this.axisX,
		...
		for (var i = 0; i < this.plotInfo.plotTypes.length; i++) {
			...
			for (var j = 0; j < plotType.plotUnits.length; j++) {
				...
				previousDataSeriesCount +=
					plotType.plotUnits[j].dataSeriesIndexes.length;
			}
		}
	};
	...
	Chart.prototype._processData = function () {
		...
		for (var i = 0; i < this.plotInfo.plotTypes.length; i++) {
			...
			for (var j = 0; j < plotType.plotUnits.length; j++) {
				...
				if (
					plotUnit.type === 'line' ||
					plotUnit.type === 'stepLine' ||
					plotUnit.type === 'spline' ||
					plotUnit.type === 'column' ||
					plotUnit.type === 'area' ||
					plotUnit.type === 'stepArea' ||
					plotUnit.type === 'splineArea' ||
					plotUnit.type === 'bar' ||
					plotUnit.type === 'bubble' ||
					plotUnit.type === 'scatter'
				)
					this._processMultiseriesPlotUnit(plotUnit);
				else if (
					plotUnit.type === 'stackedColumn' ||
					plotUnit.type === 'stackedBar' ||
					plotUnit.type === 'stackedArea'
				)
					this._processStackedPlotUnit(plotUnit);
				else if (
					plotUnit.type === 'stackedColumn100' ||
					plotUnit.type === 'stackedBar100' ||
					plotUnit.type === 'stackedArea100'
				)
					this._processStacked100PlotUnit(plotUnit);
				else if (
					plotUnit.type === 'candlestick' ||
					plotUnit.type === 'ohlc' ||
					plotUnit.type === 'rangeColumn' ||
					plotUnit.type === 'rangeBar' ||
					plotUnit.type === 'rangeArea' ||
					plotUnit.type === 'rangeSplineArea'
				)
	...
	Chart.prototype._processMultiseriesPlotUnit = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (
				dataSeries.axisPlacement === 'normal' ||
				dataSeries.axisPlacement === 'xySwapped'
			) {
				var plotAreaXMin = this.sessionVariables.axisX.internalMinimum
					? this.sessionVariables.axisX.internalMinimum
					: this._options.axisX && this._options.axisX.minimum
					? this._options.axisX.minimum
					: -Infinity;
				var plotAreaXMax = this.sessionVariables.axisX.internalMaximum
					? this.sessionVariables.axisX.internalMaximum
			...
			if (
				(dataSeries.dataPoints[i].x && dataSeries.dataPoints[i].x.getTime) ||
			...
			this.plotInfo.axisXValueType = dataSeries.xValueType = isDateTime
				? 'dateTime'
				: 'number';
		}
	...
	Chart.prototype._processStackedPlotUnit = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			for (i = 0; i < dataSeries.dataPoints.length; i++) {
				...
				plotUnit.yTotals[dataPointX] =
					(!plotUnit.yTotals[dataPointX] ? 0 : plotUnit.yTotals[dataPointX]) +
					Math.abs(dataPointY);

				if (dataPointY >= 0) {
					if (dataPointYPositiveSums[dataPointX])
						dataPointYPositiveSums[dataPointX] += dataPointY;
					else dataPointYPositiveSums[dataPointX] = dataPointY;
				} else {
					if (dataPointYNegativeSums[dataPointX])
						dataPointYNegativeSums[dataPointX] += dataPointY;
					else dataPointYNegativeSums[dataPointX] = dataPointY;
				}
			}
	...
	Chart.prototype._processStacked100PlotUnit = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			for (i = 0; i < dataSeries.dataPoints.length; i++) {
				...
				else dataPointYSums[dataPointX] = Math.abs(dataPointY);
			}
	...
	Chart.prototype.getDataPointAtXY = function (mouseX, mouseY, getClosest) {
		...
		for (var m = 0; m < results.length; m++) {
			if (
				results[m].dataSeries.type === 'line' ||
				results[m].dataSeries.type === 'stepLine' ||
				results[m].dataSeries.type === 'area' ||
				results[m].dataSeries.type === 'stepArea'
			) {
				var markerSize =
					getProperty(
						'markerSize',
						results[m].dataPoint,
						results[m].dataSeries
					) || 8;
	...
	Chart.prototype._touchEventHandler = function (ev) {
		...
		switch (ev.type) {
			...
			case 'MSPointerUp':
				mouseEvents =
					this._lastTouchEventType === 'touchstart' ||
					this._lastTouchEventType === 'MSPointerDown'
						? ['mouseup', 'click']
						: ['mouseup'];
				break;
			default:
			...
			if (type === 'mouseup') {
				...
				if (eventParam.chart.overlaidCanvas.releaseCapture)
					...
					document.body.removeEventListener(
						'mouseup',
						eventParam.chart._mouseEventHandler,
						false
					);
			}
						...
						if (this.plotInfo.axisPlacement === 'xySwapped') {
							...
							if (Math.abs(selectedRegion.y1 - selectedRegion.y2) > 1) {
								...
								var minX =
									...
									((this.axisX.maximum - this.axisX.minimum) /
										axisXProps.height) *
										(selectedRegion.y2 - axisXProps.y1);
								var maxX =
									this.axisX.maximum -
									((this.axisX.maximum - this.axisX.minimum) /
										axisXProps.height) *
										(selectedRegion.y1 - axisXProps.y1);
								...
								if (
									Math.abs(maxX - minX) >
									2 * Math.abs(this.axisX.dataInfo.minDiff)
								) {
							...
							if (Math.abs(selectedRegion.x1 - selectedRegion.x2) > 1) {
								...
								var minX =
									((this.axisX.maximum - this.axisX.minimum) /
										axisXProps.width) *
										(selectedRegion.x1 - axisXProps.x1) +
									this.axisX.minimum;
								var maxX =
									((this.axisX.maximum - this.axisX.minimum) /
										axisXProps.width) *
										(selectedRegion.x2 - axisXProps.x1) +
									this.axisX.minimum;
	...
	Chart.prototype._plotAreaMouseMove = function (x, y) {
		if (this.isDrag && this.plotInfo.axisPlacement !== 'none') {
			...
			if (
				Math.abs(dragDelta) > 2 &&
				Math.abs(dragDelta) < 8 &&
			...
			if (Math.abs(dragDelta) > 2 && (this.panEnabled || this.zoomEnabled)) {
				if (this.panEnabled) {
					this.axisX.sessionVariables.internalMinimum =
						this.dragStartPoint.xMinimum + dragValue;
					this.axisX.sessionVariables.internalMaximum =
						this.dragStartPoint.xMaximum + dragValue;
					...
					if (
						this.axisX.sessionVariables.internalMinimum <
						this.axisX._absoluteMinimum -
							convertToNumber(this.axisX.interval, this.axisX.intervalType)
					) {
						overFlow =
							this.axisX._absoluteMinimum -
							convertToNumber(this.axisX.interval, this.axisX.intervalType) -
					...
					if (this.plotInfo.axisPlacement === 'xySwapped') {
						this.overlaidCanvasCtx.fillRect(
							plotAreaBounds.x1,
							this.dragStartPoint.y,
							plotAreaBounds.x2 - plotAreaBounds.x1,
						...
						this.overlaidCanvasCtx.fillRect(
							...
							plotAreaBounds.y2 - plotAreaBounds.y1
						);
					}
	...
	Chart.prototype.preparePlotArea = function () {
		...
		if (this.axisX && yAxis) {
			plotArea.x1 =
				this.axisX.lineCoordinates.x1 < this.axisX.lineCoordinates.x2
					...
					: yAxis.lineCoordinates.x1;
			plotArea.y1 =
				this.axisX.lineCoordinates.y1 < yAxis.lineCoordinates.y1
					? this.axisX.lineCoordinates.y1
					: yAxis.lineCoordinates.y1;

			plotArea.x2 =
				this.axisX.lineCoordinates.x2 > yAxis.lineCoordinates.x2
					? this.axisX.lineCoordinates.x2
					: yAxis.lineCoordinates.x2;
			plotArea.y2 =
				this.axisX.lineCoordinates.y2 > this.axisX.lineCoordinates.y1
					? this.axisX.lineCoordinates.y2
					: yAxis.lineCoordinates.y2;
		...
		plotArea.layoutManager = new LayoutManager(
			plotArea.x1,
			plotArea.y1,
			plotArea.x2,
			plotArea.y2,
	...
	Chart.prototype.renderIndexLabels = function (targetCtx) {
		...
		for (var i = 0; i < this._indexLabels.length; i++) {
			...
			var fontColor = getProperty(
				'indexLabelFontColor',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var fontSize = getProperty(
				'indexLabelFontSize',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var fontFamily = getProperty(
				'indexLabelFontFamily',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var fontStyle = getProperty(
				'indexLabelFontStyle',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var fontWeight = getProperty(
				'indexLabelFontWeight',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var backgroundColor = getProperty(
				'indexLabelBackgroundColor',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var maxWidth = getProperty(
				'indexLabelMaxWidth',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var indexLabelWrap = getProperty(
				'indexLabelWrap',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			...
			if (
				indexLabel.dataSeries.type.indexOf('stacked') >= 0 ||
				indexLabel.dataSeries.type === 'pie' ||
				indexLabel.dataSeries.type === 'doughnut'
			)
				percentAndTotal = this.getPercentAndTotal(
					indexLabel.dataSeries,
					indexLabel.dataPoint
				);
			...
			if (
				...
				indexLabel.dataPoint.indexLabelFormatter
			)
				formatterParameter = {
					chart: this._options,
			...
			var indexLabelText = indexLabel.dataPoint.indexLabelFormatter
				...
				? this.replaceKeywordsWithValue(
						...
						indexLabel.dataSeries,
						null,
						indexLabel.indexKeyword
				  )
				: indexLabel.dataSeries.indexLabelFormatter
				? indexLabel.dataSeries.indexLabelFormatter(formatterParameter)
				: indexLabel.dataSeries.indexLabel
				? this.replaceKeywordsWithValue(
						indexLabel.dataSeries.indexLabel,
						indexLabel.dataPoint,
						indexLabel.dataSeries,
			...
			var placement = getProperty(
				'indexLabelPlacement',
				indexLabel.dataPoint,
				indexLabel.dataSeries
			);
			var orientation = getProperty(
			...
			var textBlock = new TextBlock(ctx, {
				x: 0,
				y: 0,
				maxWidth: maxWidth ? maxWidth : this.width * 0.5,
			...
			if (
				chartTypeLower.indexOf('line') >= 0 ||
				chartTypeLower.indexOf('area') >= 0 ||
				chartTypeLower.indexOf('bubble') >= 0 ||
				chartTypeLower.indexOf('scatter') >= 0
			) {
				if (
					indexLabel.dataPoint.x < axisX.minimum ||
					indexLabel.dataPoint.x > axisX.maximum ||
					indexLabel.dataPoint.y < axisY.minimum ||
					indexLabel.dataPoint.y > axisY.maximum
				)
					continue;
			} else {
				if (
			...
			if (this.plotInfo.axisPlacement === 'normal') {
				if (
					...
					chartTypeLower === 'bubble' ||
					chartTypeLower === 'scatter'
				) {
					placement = 'inside';
				}
				...
				if (placement !== 'inside') {
					...
					if (direction > 0) {
						...
						if (y > yMaxLimit - visibleHeight - marginY) {
							if (placement === 'auto') {
								y =
									Math.min(indexLabel.point.y, yMaxLimit) -
									visibleHeight -
									marginY;
							} else {
					...
					if (chartTypeLower.indexOf('range') >= 0) {
						if (direction > 0)
							mid =
								Math.max(indexLabel.bounds.y1, plotArea.y1) +
								visibleHeight / 2 +
								marginY;
						else
							mid =
								Math.min(indexLabel.bounds.y2, plotArea.y2) -
								visibleHeight / 2 -
								marginY;
					} else
						mid =
					...
					if (direction > 0) {
						...
						if (
							y < yMinLimit &&
							(chartTypeLower === 'bubble' || chartTypeLower === 'scatter')
						) {
							y = Math.max(
								indexLabel.point.y - visibleHeight - marginY,
								plotArea.y1 + marginY
							);
						}
					} else {
						...
						if (
							...
							y = Math.min(
								indexLabel.point.y + marginY,
								plotArea.y2 - visibleHeight - marginY
							);
						}
					}
				}
				...
				if (placement !== 'inside') {
					...
					if (direction < 0) {
						...
						if (x > xMaxLimit - visibleWidth - marginX) {
							if (placement === 'auto') {
								x =
									Math.min(indexLabel.point.x, xMaxLimit) -
									visibleWidth -
									marginX;
							} else {
					...
					if (chartTypeLower.indexOf('range') >= 0) {
						if (direction < 0)
							mid =
								Math.max(indexLabel.bounds.x1, plotArea.x1) +
								visibleWidth / 2 +
								marginX;
						else
							mid =
								Math.min(indexLabel.bounds.x2, plotArea.x2) -
								visibleWidth / 2 -
								marginX;
					} else
						var mid =
	...
	Chart.prototype.renderLine = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			ghostCtx.lineWidth =
				dataSeries.lineThickness > 0
					? Math.max(dataSeries.lineThickness, 4)
					: 0;
	...
	Chart.prototype.renderStepLine = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				var prevDataNull = false;
				for (i = 0; i < dataPoints.length; i++) {
					dataPointX = dataPoints[i].getTime
						? dataPoints[i].x.getTime()
						: dataPoints[i].x;
	...
	Chart.prototype.renderColumn = function (plotUnit) {
		...
		var maxBarWidth = this.dataPointMaxWidth
			? this.dataPointMaxWidth
			: Math.min(
					this.width * 0.15,
					(this.plotArea.width / plotUnit.plotType.totalDataSeries) * 0.9
			  ) << 0;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
		var barWidth =
			((((plotArea.width /
				Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) *
				Math.abs(xMinDiff)) /
				plotUnit.plotType.totalDataSeries) *
				0.9) <<
			0;

		if (barWidth > maxBarWidth) barWidth = maxBarWidth;
		else if (xMinDiff === Infinity) {
			barWidth = (maxBarWidth / plotUnit.plotType.totalDataSeries) * 0.9;
		} else if (barWidth < 1) barWidth = 1;

		ctx.save();
		if (isCanvasSupported) this._eventManager.ghostCtx.save();
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				var bevelEnabled =
					barWidth > 5 && dataSeries.bevelEnabled ? true : false;

				for (i = 0; i < dataPoints.length; i++) {
					dataPoints[i].getTime
						? (dataPointX = dataPoints[i].x.getTime())
						: (dataPointX = dataPoints[i].x);
					...
					var x1 =
						(x -
							(plotUnit.plotType.totalDataSeries * barWidth) / 2 +
							(plotUnit.previousDataSeriesCount + j) * barWidth) <<
						0;
					...
					drawRect(
						...
						0,
						null,
						bevelEnabled && dataPoints[i].y >= 0,
						dataPoints[i].y < 0 && bevelEnabled,
						false,
						false,
	...
	Chart.prototype.renderStackedColumn = function (plotUnit) {
		...
		var barWidth =
			((((plotArea.width /
				Math.abs(plotUnit.axisX.maximum - plotUnit.axisX.minimum)) *
				Math.abs(xMinDiff)) /
				plotUnit.plotType.plotUnits.length) *
				0.9) <<
			0;
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					var x1 =
						(x -
							(plotUnit.plotType.plotUnits.length * barWidth) / 2 +
							plotUnit.index * barWidth) <<
						0;
					var x2 = (x1 + barWidth) << 0;
					var y1;
					var y2;
					...
					if (dataPoints[i].y >= 0) {
						var offset = offsetPositiveY[dataPointX]
							? offsetPositiveY[dataPointX]
							: 0;
						...
						var offset = offsetNegativeY[dataPointX]
							? offsetNegativeY[dataPointX]
							: 0;
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (plotUnit.dataPointYSums[dataPointX] !== 0)
						yPercent =
							(dataPoints[i].y / plotUnit.dataPointYSums[dataPointX]) * 100;
					else yPercent = 0;
					...
					y =
						plotUnit.axisY.conversionParameters.reference +
						plotUnit.axisY.conversionParameters.pixelPerUnit *
							(yPercent - plotUnit.axisY.conversionParameters.minimum);
	...
	Chart.prototype.renderBar = function (plotUnit) {
		...
		var maxBarWidth = this.dataPointMaxWidth
			...
			: Math.min(
					this.height * 0.15,
					(this.plotArea.height / plotUnit.plotType.totalDataSeries) * 0.9
			  ) << 0;
		var xMinDiff = plotUnit.axisX.dataInfo.minDiff;
	...
	Chart.prototype.renderArea = function (plotUnit) {
		...
		function closeArea() {
			...
			if (plotUnit.axisY.minimum <= 0 && plotUnit.axisY.maximum >= 0) {
				baseY = yZeroToPixel;
			} else if (plotUnit.axisY.maximum < 0) baseY = axisYProps.y1;
			else if (plotUnit.axisY.minimum > 0) baseY = axisXProps.y2;
	...
	Chart.prototype.renderStackedArea = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (allXValues.length > 0) {
				...
				for (i = 0; i < allXValues.length; i++) {
					...
					else dataPoint = { x: dataPointX, y: 0 };
					...
					var y =
						plotUnit.axisY.conversionParameters.reference +
						plotUnit.axisY.conversionParameters.pixelPerUnit *
							(dataPoint.y - plotUnit.axisY.conversionParameters.minimum);
					...
					if (isFirstDataPointInPlotArea) {
						...
						if (i % 250 == 0) {
							...
							while (currentBaseValues.length > 0) {
								...
								if (isCanvasSupported) ghostCtx.lineTo(point.x, point.y);
							}
					...
					if (dataSeries.dataPointIndexes[dataPointX] >= 0) {
						var id =
							dataSeries.dataPointIds[dataSeries.dataPointIndexes[dataPointX]];
						this._eventManager.objectMap[id] = {
							id: id,
							objectType: 'dataPoint',
							dataSeriesIndex: dataSeriesIndex,
					...
					//Render Marker
					if (
						dataSeries.dataPointIndexes[dataPointX] >= 0 &&
		...
		if (isCanvasSupported) ghostCtx.restore();
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (allXValues.length > 0) {
				...
				for (i = 0; i < allXValues.length; i++) {
					...
					if (plotUnit.dataPointYSums[dataPointX] !== 0)
						yPercent =
							(dataPoint.y / plotUnit.dataPointYSums[dataPointX]) * 100;
					else yPercent = 0;
	...
	Chart.prototype.renderBubble = function (plotUnit) {
		...
		var maxArea = Math.max(
			Math.pow((Math.min(plotArea.height, plotArea.width) * 0.25) / 2, 2) *
				Math.PI,
			minArea
		);
			...
			if (dataPoints.length > 0) {
				...
				for (var i = 0; i < dataPoints.length; i++) {
					...
					var area =
						maxZ === minZ
							? maxArea / 2
							: minArea + ((maxArea - minArea) / (maxZ - minZ)) * (z - minZ);
					...
					if (
						...
						this._indexLabels.push({
							...
							direction: 1,
							bounds: {
								x1: x - markerProps.size / 2,
								y1: y - markerProps.size / 2,
								x2: x + markerProps.size / 2,
								y2: y + markerProps.size / 2
							},
							color: color
						});
					}
	...
	Chart.prototype.renderScatter = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			if (dataPoints.length > 0) {
				...
				var maxArea =
					Math.pow((Math.min(plotArea.height, plotArea.width) * 0.3) / 2, 2) *
					Math.PI;
				...
				for (var i = 0; i < dataPoints.length; i++) {
					...
					if (
						Math.sqrt(
							(prevDataPointX - x) * (prevDataPointX - x) +
								(prevDataPointY - y) * (prevDataPointY - y)
						) < Math.min(markerProps.size, 5) &&
						dataPoints.length >
			...
			if (dataPoints.length > 0) {
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (
						...
						typeof dataPoints[i].y[2] !== 'number' ||
						typeof dataPoints[i].y[3] !== 'number'
					)
						continue;
					...
					var borderThickness = Math.round(Math.max(1, barWidth * 0.15));
					...
					if (dataSeries.type === 'candlestick') {
						...
						drawRect(
							...
							x2,
							Math.max(y1, y4),
							dataPoints[i].y[0] <= dataPoints[i].y[3]
	...
	Chart.prototype.renderPie = function (plotUnit) {
		...
		var outerRadius =
			dataSeries.indexLabelPlacement === 'inside'
				...
				: (Math.min(plotArea.width, plotArea.height) * 0.8) / 2;
		...
		function initLabels() {
			...
			for (j = 0; j < dataPoints.length; j++) {
				...
				dataPointEO.indexLabelBackgroundColor = dataPoint.indexLabelBackgroundColor
				...
				dataPointEO.endAngle =
					dataPointEO.startAngle +
					((2 * Math.PI) / sum) * Math.abs(dataPoint.y);
		...
		function animate(fractionComplete) {
			...
			var maxAngle =
				dataPointEOs[0].startAngle + 2 * Math.PI * fractionComplete;
		...
		function areDataPointsTooClose(first, second) {
			...
			var label2 = {
				...
				y1:
					second.indexLabelTextBlock.y - second.indexLabelTextBlock.height / 2,
		...
		function rePositionLabels(dataPointIndex, offset) {
			...
			if (dataPointIndex >= 0 && dataPointIndex < dataPoints.length) {
				...
				//Check if the offset falls within the bounds (labelYMin, labelYMax, tangential bounds) without considering overlap. Else use the closest offset that is possible - validOffset.
				{
					...
					indexLabelAngleWhenTangent = Math.acos(
						outerRadius / indexLabelRadius
					);
					...
					if (otherDataPointIndex !== null) {
						...
						var distanceFromOtherLabel =
							...
							(dataPointEOs[otherDataPointIndex].indexLabelTextBlock.y +
								dataPointEOs[otherDataPointIndex].indexLabelTextBlock.height /
									2);

						if (
							distanceFromOtherLabel - tempOffset <
		...
		function positionLabels() {
			...
			for (j = 0; j < 10 && (j < 1 || deltaR > 0); j++) {
					...
					for (i = 0; i < dataPoints.length; i++) {
						...
						if (distanceFromNextLabel < 0) {
							...
							var upWardsOffset =
								...
								dataPointsBelow;
							var downWardsOffset =
								-1 * (distanceFromNextLabel - upWardsOffset);
				...
				for (i = 0; i < dataPoints.length; i++) {
					...
					if (yOverFlow > 0) {
						if (
							Math.abs(dataPointEO.indexLabelTextBlock.x - center.x) <
							outerRadius
						) {
		...
		this.pieDoughnutClickHandler = function (e) {
			...
			if (dataSeries.dataPoints.length > 1) {
				_this._animator.animate(0, explodeDuration, function (
					fractionComplete
				) {
	...
	Title.prototype.render = function () {
		...
		var rightOffset =
			...
			this.verticalAlign === 'top'
				? 22
				: 0; //So that Title doesn't overlap menu button.
		...
		if (this.verticalAlign === 'top' || this.verticalAlign === 'bottom') {
			if (this.maxWidth === null)
				this.maxWidth =
					freespace.width -
					containerMargin * 2 -
					rightOffset * (this.horizontalAlign === 'center' ? 2 : 1);

			maxHeight = freespace.height * 0.5 - this.margin - containerMargin;
			angle = 0;
			...
			if (this.horizontalAlign === 'left' || this.horizontalAlign === 'right') {
				...
				maxHeight = freespace.width * 0.5 - this.margin - containerMargin;
			} else if (this.horizontalAlign === 'center') {
				if (this.maxWidth === null)
					this.maxWidth = freespace.width - containerMargin * 2;

				maxHeight = freespace.height * 0.5 - containerMargin * 2;
			}
		}
		...
		if (!this.wrap)
			maxHeight = Math.min(
				maxHeight,
				Math.max(this.fontSize * 1.5, this.fontSize + this.padding * 2.5)
			);
		...
		if (this.verticalAlign === 'top' || this.verticalAlign === 'bottom') {
			...
			if (this.horizontalAlign === 'left') {
				...
				left =
					freespace.x2 - containerMargin - textBlockSize.width - rightOffset;
			}
			...
			if (this.horizontalAlign === 'left') {
				...
				top =
					freespace.y2 -
					containerMargin -
					(this.maxWidth / 2 - textBlockSize.width / 2);
				angle = -90;
	...
	function Legend(chart, options, theme) {
		...
		this.lineHeight = getFontHeightInPixels(
			this.fontFamily,
			this.fontSize,
			this.fontWeight
		);
	...
	Legend.prototype.render = function () {
		...
		if (this.verticalAlign === 'top' || this.verticalAlign === 'bottom') {
			...
			maxWidth = this.maxWidth !== null ? this.maxWidth : freeSpace.width * 0.7;
			maxHeight =
				this.maxHeight !== null ? this.maxHeight : freeSpace.height * 0.5;
		} else if (this.verticalAlign === 'center') {
			this.orientation = 'vertical';
			position = this.horizontalAlign;

			maxWidth = this.maxWidth !== null ? this.maxWidth : freeSpace.width * 0.5;
			maxHeight =
				this.maxHeight !== null ? this.maxHeight : freeSpace.height * 0.7;
		}
		...
		for (var i = 0; i < this.dataSeries.length; i++) {
			...
			if (
				dataSeries.type !== 'pie' &&
				dataSeries.type !== 'doughnut' &&
				dataSeries.type !== 'funnel'
			) {
				var markerType = dataSeries.legendMarkerType
					? dataSeries.legendMarkerType
					: (dataSeries.type === 'line' ||
							dataSeries.type === 'stepLine' ||
							dataSeries.type === 'spline' ||
							dataSeries.type === 'scatter' ||
					...
					? dataSeries.markerType
					: DataSeries.getDefaultLegendMarker(dataSeries.type);
				var legendText = dataSeries.legendText
					...
					: dataSeries.name;
				var markerColor = dataSeries.legendMarkerColor
					? dataSeries.legendMarkerColor
					: dataSeries.markerColor
					? dataSeries.markerColor
					: dataSeries._colorSet[0];
				var markerSize =
					!dataSeries.markerSize &&
					(dataSeries.type === 'line' ||
						dataSeries.type === 'stepLine' ||
						dataSeries.type === 'spline')
						? 0
						: this.lineHeight * 0.6;
				var markerBorderColor = dataSeries.legendMarkerBorderColor
					? dataSeries.legendMarkerBorderColor
					: dataSeries.markerBorderColor;
				var markerBorderThickness = dataSeries.legendMarkerBorderThickness
					? dataSeries.legendMarkerBorderThickness
					: dataSeries.markerBorderThickness
					? Math.max(1, Math.round(markerSize * 0.2))
					: 0;
				var lineColor = dataSeries._colorSet[0];

				legendText = this.chart.replaceKeywordsWithValue(
					legendText,
					dataSeries.dataPoints[0],
					dataSeries,
					i
				);
				var item = {
				...
				for (
					...
					dataPointIndex < dataSeries.dataPoints.length;
					dataPointIndex++
				) {
					var dataPoint = dataSeries.dataPoints[dataPointIndex];

					var markerType = dataPoint.legendMarkerType
						? dataPoint.legendMarkerType
						: dataSeries.legendMarkerType
						? dataSeries.legendMarkerType
						: DataSeries.getDefaultLegendMarker(dataSeries.type);
					var legendText = dataPoint.legendText
						...
						: dataPoint.name
						? dataPoint.name
						: 'DataPoint: ' + (dataPointIndex + 1);
					var markerColor = dataPoint.legendMarkerColor
						? dataPoint.legendMarkerColor
						: dataSeries.legendMarkerColor
						? dataSeries.legendMarkerColor
						: dataPoint.color
						? dataPoint.color
						...
						? dataSeries.color
						: dataSeries._colorSet[
								dataPointIndex % dataSeries._colorSet.length
						  ];
					var markerSize = this.lineHeight * 0.6;
					var markerBorderColor = dataPoint.legendMarkerBorderColor
						? dataPoint.legendMarkerBorderColor
						: dataSeries.legendMarkerBorderColor
						? dataSeries.legendMarkerBorderColor
						: dataPoint.markerBorderColor
						? dataPoint.markerBorderColor
						: dataSeries.markerBorderColor;
					var markerBorderThickness = dataPoint.legendMarkerBorderThickness
					...
					if (
						dataPoint.showInLegend ||
						(dataSeries.showInLegend && dataPoint.showInLegend !== false)
					) {
						items.push(item);
					}
				}
		...
		if (items.length > 0) {
			...
			markerSize = markerSize === 0 ? this.lineHeight * 0.6 : markerSize;
			textMaxWidth = textMaxWidth - (markerSize + this.horizontalSpacing * 0.1);
			...
			for (var i = 0; i < items.length; i++) {
				...
				if (
					item.chartType === 'line' ||
					item.chartType === 'spline' ||
					item.chartType === 'stepLine'
				) {
					textMaxWidth = textMaxWidth - 2 * (this.lineHeight * 0.1);
				}
						...
						row.width +
							Math.round(
								item.textBlock.width +
									this.horizontalSpacing * 0.1 +
									markerSize +
									(row.width === 0 ? 0 : this.horizontalSpacing) +
									(item.chartType === 'line' ||
									item.chartType === 'spline' ||
									item.chartType === 'stepLine'
										? 2 * (this.lineHeight * 0.1)
		...
		if (this.verticalAlign === 'top') {
			if (this.horizontalAlign === 'left') left = freeSpace.x1;
			else if (this.horizontalAlign === 'right')
				left = freeSpace.x2 - this.width;
			else left = freeSpace.x1 + freeSpace.width / 2 - this.width / 2;
		...
		for (var i = 0; i < rows.length; i++) {
			...
			for (var itemIndex = 0; itemIndex < row.items.length; itemIndex++) {
				...
				var itemX =
					item.textBlock.x +
					left +
					(itemIndex === 0 ? markerSize * 0.2 : this.horizontalSpacing);
				...
				if (
					...
					this.ctx.beginPath();
					this.ctx.moveTo(
						itemX - this.lineHeight * 0.1,
						itemY + this.lineHeight / 2
					);
					this.ctx.lineTo(
						itemX + this.lineHeight * 0.7,
						itemY + this.lineHeight / 2
					);
					this.ctx.stroke();

					ghostX -= this.lineHeight * 0.1;
				}
				...
				RenderHelper.drawMarker(
					...
					item.markerSize,
					item.markerColor,
					item.markerBorderColor,
					item.markerBorderThickness
				);

				item.textBlock.x = itemX + this.horizontalSpacing * 0.1 + markerSize;
				...
				if (
					...
					item.textBlock.x = item.textBlock.x + this.lineHeight * 0.1;
				}
				...
				this.ghostCtx.fillRect(
					ghostX,
					item.textBlock.y,
					item.textBlock.x + item.textBlock.width - ghostX,
					item.textBlock.height
				);

				item.x1 = this.chart._eventManager.objectMap[item.id].x1 = ghostX;
				item.y1 = this.chart._eventManager.objectMap[item.id].y1 =
					item.textBlock.y;
				item.x2 = this.chart._eventManager.objectMap[item.id].x2 =
					item.textBlock.x + item.textBlock.width;
				item.y2 = this.chart._eventManager.objectMap[item.id].y2 =
					item.textBlock.y + item.textBlock.height;
			}
	...
	function DataSeries(chart, options, theme, index, id) {
		...
		if (this.fillOpacity === null) {
			if (this.type.match(/area/i)) this.fillOpacity = 0.7;
			else this.fillOpacity = 1;
		}
	...
	DataSeries.prototype.getDataPointAtX = function (x, findClosest) {
		...
		if (this.chart.plotInfo.axisPlacement !== 'none') {
			...
			if (xRange > 0)
				searchStartIndex = Math.min(
					Math.max(
						(((this.dataPoints.length - 1) / xRange) *
		...
		while (true) {
			...
			if (i >= 0 && i < this.dataPoints.length) {
				...
				if (xDistance <= minimumXDistance) minimumXDistance = xDistance;
				else {
					if (direction > 0) forwardMissCount++;
					else backwardMissCount++;
				}

				if (forwardMissCount > maxMissCount && backwardMissCount > maxMissCount)
					break;
			} else if (
				searchStartIndex - j < 0 &&
				searchStartIndex + j >= this.dataPoints.length
			)
				break;
		...
		if (!findClosest && searchResult.dataPoint.x === x) return searchResult;
				...
				if (visualInfo) {
					switch (this.type) {
						...
						case 'rangeBar':
							if (
								...
								results.push({
									...
									dataSeries: this,
									distance: Math.min(
										Math.abs(visualInfo.x1 - x),
										Math.abs(visualInfo.x2 - x),
										Math.abs(visualInfo.y1 - y),
										Math.abs(visualInfo.y2 - y)
									)
									//distance:0
								});
						...
						case 'scatter':
							...
							distance = Math.sqrt(
								Math.pow(visualInfo.x1 - x, 2) + Math.pow(visualInfo.y1 - y, 2)
							);
						...
						case 'rangeSplineArea':
							...
							distance = Math.min(
								Math.sqrt(
									Math.pow(visualInfo.x1 - x, 2) +
										Math.pow(visualInfo.y1 - y, 2)
								),
								Math.sqrt(
									Math.pow(visualInfo.x1 - x, 2) +
										Math.pow(visualInfo.y2 - y, 2)
								)
							);
							if (distance <= snapDistance) {
								results.push({
						...
						case 'doughnut':
							...
							var innerRadius =
								this.type === 'doughnut' ? 0.6 * visualInfo.radius : 0;
							...
							if (distance < visualInfo.radius && distance > innerRadius) {
								...
								angle = Number(
									(((((angle / Math.PI) * 180) % 360) + 360) % 360).toFixed(12)
								);
								//console.log(angle);

								var startAngle = Number(
									(
										((((visualInfo.startAngle / Math.PI) * 180) % 360) + 360) %
										360
									).toFixed(12)
								);
								var endAngle = Number(
									(
										((((visualInfo.endAngle / Math.PI) * 180) % 360) + 360) %
										360
									).toFixed(12)
								);
						...
						case 'candlestick':
							if (
								(x >= visualInfo.x1 - visualInfo.borderThickness / 2 &&
									x <= visualInfo.x2 + visualInfo.borderThickness / 2 &&
									y >= visualInfo.y2 - visualInfo.borderThickness / 2 &&
									y <= visualInfo.y3 + visualInfo.borderThickness / 2) ||
								(Math.abs(visualInfo.x2 - x + visualInfo.x1 - x) <
									visualInfo.borderThickness &&
									y >= visualInfo.y1 &&
									y <= visualInfo.y4)
							) {
								results.push({
						...
						case 'ohlc':
							if (
								(Math.abs(visualInfo.x2 - x + visualInfo.x1 - x) <
									...
									y <= visualInfo.y3) ||
								(x >= visualInfo.x1 &&
									x <= (visualInfo.x2 + visualInfo.x1) / 2 &&
									y >= visualInfo.y1 - visualInfo.borderThickness / 2 &&
									y <= visualInfo.y1 + visualInfo.borderThickness / 2) ||
								(x >= (visualInfo.x1 + visualInfo.x2) / 2 &&
									x <= visualInfo.x2 &&
									y >= visualInfo.y4 - visualInfo.borderThickness / 2 &&
									y <= visualInfo.y4 + visualInfo.borderThickness / 2)
							) {
								results.push({
									dataPoint: dataPoint,
					...
					if (
						foundDataPoint ||
						(forwardMissCount > maxMissCount &&
							backwardMissCount > maxMissCount)
					)
						break;
				}
			} else if (
	...
	DataSeries.prototype.getMarkerProperties = function (index, x, y, ctx) {
		...
		var markerColor = dataPoints[index].markerColor
			? dataPoints[index].markerColor
			: dataSeries.markerColor
			? dataSeries.markerColor
			...
			: dataSeries.color
			? dataSeries.color
			: dataSeries._colorSet[index % dataSeries._colorSet.length];
		var markerBorderColor = dataPoints[index].markerBorderColor
			? dataPoints[index].markerBorderColor
			: dataSeries.markerBorderColor
			? dataSeries.markerBorderColor
			: null;
		var markerBorderThickness = dataPoints[index].markerBorderThickness
			? dataPoints[index].markerBorderThickness
			: dataSeries.markerBorderThickness
			? dataSeries.markerBorderThickness
			: null;
		var markerType = dataPoints[index].markerType
			...
			: dataSeries.markerType;
		var markerSize = dataPoints[index].markerSize
	...
	Axis.prototype.createLabels = function () {
		...
		if (this._position === 'bottom' || this._position === 'top') {
			intervalInPixels =
				(this.lineCoordinates.width / Math.abs(this.maximum - this.minimum)) *
				this.interval;
			...
			intervalInPixels =
				(this.lineCoordinates.height / Math.abs(this.maximum - this.minimum)) *
				this.interval;
		...
		if (
			...
			for (
				i = this.intervalstartTimePercent;
				i < endPoint;
	...
	Axis.setLayoutAndRender = function (
		...
		if (
			axisY &&
			axisY2 &&
			typeof axisY._options.maximum === 'undefined' &&
			typeof axisY._options.minimum === 'undefined' &&
			typeof axisY._options.interval === 'undefined' &&
			typeof axisY2._options.maximum === 'undefined' &&
			typeof axisY2._options.minimum === 'undefined' &&
			typeof axisY2._options.interval === 'undefined'
		) {
		...
		var axisYlineThickness = axisY
			? axisY.lineThickness
				? axisY.lineThickness
				: 0
			: 0;
		var axisY2lineThickness = axisY2
			? axisY2.lineThickness
				? axisY2.lineThickness
				: 0
			: 0;

		var axisYGridThickness = axisY
			? axisY.gridThickness
				? axisY.gridThickness
				: 0
			: 0;
		var axisY2GridThickness = axisY2
			? axisY2.gridThickness
				? axisY2.gridThickness
				: 0
			: 0;
		...
		if (typeof this._options.interval === 'undefined') {
			if (this._position === 'bottom' || this._position === 'top') {
				...
				for (i = 0; i < this._labels.length; i++) {
					...
					if (label.position < this.minimum || label.stripLine)
	...
	Axis.prototype.renderStripLinesOfThicknessType = function (thicknessType) {
		...
		for (i = 0; i < this.stripLines.length; i++) {
			...
			if (
				thicknessType === 'pixel' &&
				(stripLine.value < this.minimum || stripLine.value > this.maximum)
			)
				continue;
	...
	Axis.prototype.renderGrid = function () {
		...
		if (this._position === 'bottom' || this._position === 'top') {
			for (i = 0; i < this._labels.length && !this._labels[i].stripLine; i++) {
				if (
					this._labels[i].position < this.minimum ||
					this._labels[i].position > this.maximum
				)
					continue;
	...
	Axis.prototype.getXValueAt = function (pixel) {
		...
		if (this._position === 'left') {
			xval =
				((this.chart.axisX.maximum - this.chart.axisX.minimum) /
	...
	Axis.prototype.calculateValueToPixelconversionParameters = function (value) {
		...
		if (this._position === 'bottom' || this._position === 'top') {
			conversionParameters.pixelPerUnit =
				width / Math.abs(this.maximum - this.minimum);
		...
		if (this._position === 'left' || this._position === 'right') {
			conversionParameters.pixelPerUnit =
				(-1 * height) / Math.abs(this.maximum - this.minimum);
	...
	Axis.prototype.calculateAxisParameters = function () {
		...
		if (this.type === 'axisX') {
			min =
				this.sessionVariables.internalMinimum !== null
					? this.sessionVariables.internalMinimum
					: this.dataInfo.viewPortMin;
			max =
				this.sessionVariables.internalMaximum !== null
					? this.sessionVariables.internalMaximum
			...
			min =
				typeof this._options.minimum === 'undefined' ||
				this._options.minimum === null
					? this.dataInfo.viewPortMin
					: this._options.minimum;
			max =
				typeof this._options.maximum === 'undefined' ||
				this._options.maximum === null
					? this.dataInfo.viewPortMax
					: this._options.maximum;
		...
		if (
			...
			else this.maximum = max + minDiff / 2;
			...
			this.intervalstartTimePercent = this.getLabelStartPoint(
				new Date(this.minimum),
				this.intervalType,
				this.interval
			);
		} else {
			...
			if (this.maximum === 0 && this.minimum === 0) {
				...
				if (this._options && typeof this._options.interval === 'undefined') {
					this.interval = Axis.getNiceNumber(
						(this.maximum - this.minimum) / (noTicks - 1),
						true
					);
				}
			}
			...
			if (this.type === 'axisX') {
				...
				this.intervalstartTimePercent =
					Math.floor((this.minimum + this.interval * 0.2) / this.interval) *
					this.interval;
			} else if (this.type === 'axisY') {
		...
		if (this.type === 'axisX') {
			this._absoluteMinimum =
				this._options && typeof this._options.minimum !== 'undefined'
					? this._options.minimum
					: this.dataInfo.min - minDiff / 2;
			this._absoluteMaximum =
				this._options && typeof this._options.maximum !== 'undefined'
					? this._options.maximum
					: this.dataInfo.max + minDiff / 2;
		}
	...
	Axis.prototype.getLabelStartPoint = function () {
		...
		var minimum =
			Math.floor(this.minimum / intervalInMilliseconds) *
	...
	StripLine.prototype.render = function () {
		...
		if (lineWidth > 0) {
			...
			if (
				...
				this.parent._position === 'left' ||
				this.parent._position === 'right'
			) {
				var stripY =
					this.ctx.lineWidth % 2 === 1 ? (xy.y << 0) + 0.5 : xy.y << 0;
	...
	ToolTip.prototype._initialize = function () {
		if (this.enabled) {
			...
			toolTipHtml +=
				'font-style: ' + (isCanvasSupported ? 'italic;' : 'normal;');
		...
		if (
			...
			if (this.chart.plotInfo.axisPlacement === 'xySwapped') {
				x =
					((this.chart.axisX.maximum - this.chart.axisX.minimum) /
						this.chart.axisX.lineCoordinates.height) *
						(this.chart.axisX.lineCoordinates.y2 - mouseY) +
					this.chart.axisX.minimum;
			} else {
				x =
					((this.chart.axisX.maximum - this.chart.axisX.minimum) /
						this.chart.axisX.lineCoordinates.width) *
						(mouseX - this.chart.axisX.lineCoordinates.x1) +
					this.chart.axisX.minimum;
			}
			...
			for (i = 0; i < nearbyEntries.length; i++) {
				if (
					nearbyEntries[i].dataPoint.x.valueOf() ===
					closest.dataPoint.x.valueOf()
				)
			...
			if (dataPointInfo) {
				...
				if (
					id > 0 &&
					typeof this.chart._eventManager.objectMap[id] !== 'undefined'
				) {
					...
					this.currentDataPointIndex =
						eventObject.dataPointIndex >= 0 ? eventObject.dataPointIndex : -1;
					//}
			...
			if (this.currentSeriesIndex >= 0) {
				...
				if (this.currentDataPointIndex >= 0) {
					...
					var x =
						((this.chart.axisX.maximum - this.chart.axisX.minimum) /
							this.chart.axisX.lineCoordinates.width) *
							(mouseX - this.chart.axisX.lineCoordinates.x1) +
						this.chart.axisX.minimum.valueOf();
				...
				if (entry.dataPoint.y !== null) entries.push(entry);
			}
		}
				...
				if (toolTipInnerHtml !== null) {
					...
					try {
						...
						this.contentDiv.style.borderRightColor = this.contentDiv.style.borderLeftColor = this.contentDiv.style.borderColor = this
							.borderColor
							? this.borderColor
							: entries[0].dataPoint.color
							? entries[0].dataPoint.color
							: entries[0].dataSeries.color
							? entries[0].dataSeries.color
							: entries[0].dataSeries._colorSet[
									entries[0].index % entries[0].dataSeries._colorSet.length
							  ];

						this.contentDiv.style.borderWidth =
							this.borderThickness || this.borderThickness === 0
								? this.borderThickness + 'px'
								: 2 + 'px';

						this.contentDiv.style.borderRadius =
							this.cornerRadius || this.cornerRadius === 0
					...
					if (
						entries[0].dataSeries.type === 'pie' ||
						entries[0].dataSeries.type === 'doughnut' ||
						entries[0].dataSeries.type === 'funnel' ||
						entries[0].dataSeries.type === 'bar' ||
						entries[0].dataSeries.type === 'rangeBar' ||
						entries[0].dataSeries.type === 'stackedBar' ||
						entries[0].dataSeries.type === 'stackedBar100'
					) {
						toolTipLeft = mouseX - 10 - this.container.clientWidth;
					} else {
						toolTipLeft =
							((this.chart.axisX.lineCoordinates.width /
								Math.abs(this.chart.axisX.maximum - this.chart.axisX.minimum)) *
								Math.abs(entries[0].dataPoint.x - this.chart.axisX.minimum) +
					...
					if (
						...
						toolTipLeft = Math.max(
							0,
							this.chart._container.clientWidth - this.container.clientWidth
						);
					...
					if (
						entries.length === 1 &&
						!this.shared &&
						(entries[0].dataSeries.type === 'line' ||
							entries[0].dataSeries.type === 'stepLine' ||
							entries[0].dataSeries.type === 'spline' ||
							entries[0].dataSeries.type === 'area' ||
							entries[0].dataSeries.type === 'stepArea' ||
							entries[0].dataSeries.type === 'splineArea' ||
							entries[0].dataSeries.type === 'stackedArea' ||
							entries[0].dataSeries.type === 'stackedArea100')
					) {
						toolTipBottom =
							(entries[0].dataSeries.axisY.lineCoordinates.y2 -
								(entries[0].dataSeries.axisY.lineCoordinates.height /
									Math.abs(
										entries[0].dataSeries.axisY.maximum -
											entries[0].dataSeries.axisY.minimum
									)) *
									Math.abs(
										entries[0].dataPoint.y - entries[0].dataSeries.axisY.minimum
									) +
								0.5) <<
							0;
					} else if (
						...
						toolTipBottom =
							(entries[0].dataSeries.axisX.lineCoordinates.y2 -
								(entries[0].dataSeries.axisX.lineCoordinates.height /
									Math.abs(
										entries[0].dataSeries.axisX.maximum -
											entries[0].dataSeries.axisX.minimum
									)) *
									Math.abs(
										entries[0].dataPoint.x - entries[0].dataSeries.axisX.minimum
									) +
								0.5) <<
							0;
					} else {
					...
					if (toolTipBottom + this.container.clientHeight + 5 > 0) {
						toolTipBottom -=
							toolTipBottom + this.container.clientHeight + 5 - 0;
					}
	...
	ToolTip.prototype.highlightObjects = function (entries) {
		...
		for (var i = 0; i < entries.length; i++) {
			...
			var eventObject = this.chart._eventManager.objectMap[
				entry.dataSeries.dataPointIds[entry.index]
			];

			if (
				!eventObject ||
				!eventObject.objectType ||
				eventObject.objectType !== 'dataPoint'
			)
				continue;
			...
			if (
				dataPoint.highlightEnabled !== false &&
				(dataSeries.highlightEnabled === true ||
				...
				if (
					...
					var markerProps = dataSeries.getMarkerProperties(
						index,
						eventObject.x1,
						eventObject.y1,
					...
					markerProps.borderThickness =
						markerProps.borderThickness || Math.ceil(markerProps.size * 0.1);
					...
					dataSeries.type === 'stackedColumn' ||
					dataSeries.type === 'stackedColumn100' ||
					dataSeries.type === 'bar' ||
					dataSeries.type === 'rangeBar' ||
					dataSeries.type === 'stackedBar' ||
					dataSeries.type === 'stackedBar100' ||
					...
					drawSegment(
						...
						'white',
						dataSeries.type,
						eventObject.startAngle,
						eventObject.endAngle,
						0.3
					);
					...
					offset = overlaidCanvasCtx.lineWidth % 2 === 0 ? 0 : 0.5;

					overlaidCanvasCtx.beginPath();
					overlaidCanvasCtx.moveTo(eventObject.x3 - offset, eventObject.y2);
					overlaidCanvasCtx.lineTo(
						eventObject.x3 - offset,
						Math.min(eventObject.y1, eventObject.y4)
					);
					overlaidCanvasCtx.stroke();

					overlaidCanvasCtx.beginPath();
					overlaidCanvasCtx.moveTo(
						eventObject.x3 - offset,
						Math.max(eventObject.y1, eventObject.y4)
					);
	...
	ToolTip.prototype.getToolTipInnerHTML = function (e) {
		...
		for (var i = 0; i < entries.length; i++) {
			if (
				entries[i].dataSeries.toolTipContent ||
				entries[i].dataPoint.toolTipContent
			) {
		...
		if (
			isToolTipDefinedInData &&
			((this.content && typeof this.content === 'function') ||
			...
			toolTipInnerHtml = this.contentFormatter
				? this.contentFormatter(param)
				: this.content(param);
		} else {
				...
				for (var i = 0; i < entries.length; i++) {
					...
					if (i === 0 && isToolTipDefinedInData && !this.content) {
						toolTipInnerHtmlPrefix +=
							typeof this.chart.axisX.labels[dataPoint.x] !== 'undefined'
								? this.chart.axisX.labels[dataPoint.x]
								: '{x}';
						toolTipInnerHtmlPrefix += '</br>';
						toolTipInnerHtmlPrefix = this.chart.replaceKeywordsWithValue(
							toolTipInnerHtmlPrefix,
							dataPoint,
							dataSeries,
							index
					...
					if (
						dataPoint.toolTipContent === null ||
						(typeof dataPoint.toolTipContent === 'undefined' &&
							dataSeries._options.toolTipContent === null)
					)
						continue;
					...
					if (this.reversed === true) {
						toolTipInnerHtml =
							this.chart.replaceKeywordsWithValue(
								toolTipContent,
								dataPoint,
								dataSeries,
								index
						...
						toolTipInnerHtml += this.chart.replaceKeywordsWithValue(
							toolTipContent,
							dataPoint,
							dataSeries,
							index
	...
	ToolTip.prototype.hide = function (resetOverlayedCanvas) {
		...
		resetOverlayedCanvas =
			typeof resetOverlayedCanvas === 'undefined' ? true : resetOverlayedCanvas;
	...
	Chart.prototype.getPercentAndTotal = function (ds, dp) {
		...
		if (ds.type.indexOf('stacked') >= 0) {
			...
			if (!isNaN(dp.y)) percent = (dp.y / total) * 100;
			else percent = 0;
		}
	...
	Chart.prototype.replaceKeywordsWithValue = function (
		str,
		dp,
		ds,
		dpIndex,
		...
		var chart = this;
		indexKeywordValue =
			typeof indexKeywordValue === 'undefined' ? 0 : indexKeywordValue;

		if (
			(ds.type.indexOf('stacked') >= 0 ||
				ds.type === 'pie' ||
				ds.type === 'doughnut') &&
			(str.indexOf('#percent') >= 0 || str.indexOf('#total') >= 0)
		) {
			var percent = '#percent';
			var total = '#total';
			var dpX = null;
			...
			percent = isNaN(percentAndTotal.percent)
				? percent
				: percentAndTotal.percent;
			...
			do {
				...
				if (ds.percentFormatString)
					...
					var numberOfDecimals = Math.max(
						Math.ceil(Math.log(1 / Math.abs(percent)) / Math.LN10),
						2
					);
				...
				str = str.replace(
					'#percent',
					numberFormat(percent, percentFormatString, chart._cultureInfo)
				);
				str = str.replace(
					'#total',
					numberFormat(
						total,
						ds.yValueFormatString ? ds.yValueFormatString : '#,##0.########'
					)
				);
		...
		var fcn = function ($0) {
			...
			if (key === 'color') {
				return dp.color
					...
					: ds.color
					? ds.color
					: ds._colorSet[dpIndex % ds._colorSet.length];
			}

			if (dp.hasOwnProperty(key)) obj = dp;
			else if (ds.hasOwnProperty(key)) obj = ds;
			else return '';
			...
			if (key === 'x') {
				if (chart.axisX && chart.plotInfo.axisXValueType === 'dateTime')
					return dateFormat(
						value,
						dp.xValueFormatString
							? dp.xValueFormatString
							: ds.xValueFormatString
							? ds.xValueFormatString
							: chart.axisX && chart.axisX.valueFormatString
							? chart.axisX.valueFormatString
							: 'DD MMM YY',
						chart._cultureInfo
					);
				else
				...
				return numberFormat(
					value,
					dp.yValueFormatString
						? dp.yValueFormatString
						: ds.yValueFormatString
						? ds.yValueFormatString
						: '#,##0.########',
					chart._cultureInfo
				);
				...
				return numberFormat(
					value,
					dp.zValueFormatString
						? dp.zValueFormatString
						: ds.zValueFormatString
						? ds.zValueFormatString
						: '#,##0.########',
					chart._cultureInfo
				);
	...
	Animator.prototype.animate = function (
		...
		while (this.animations.length > 0) {
			...
			if (animation.startTime <= now) {
				fractionComplete = easingFunction(
					Math.min(now - animation.startTime, animation.duration),
					0,
					1,
					animation.duration
				);
	...
	var AnimationHelper = {
		yScaleAnimation: function (fractionComplete, animationInfo) {
			...
			ctx.drawImage(
				...
				0,
				offsetY,
				ctx.canvas.width / devicePixelBackingStoreRatio,
		...
		xScaleAnimation: function (fractionComplete, animationInfo) {
			...
			ctx.drawImage(
				...
				offsetX,
				0,
				(fractionComplete * ctx.canvas.width) / devicePixelBackingStoreRatio,
		...
		xClipAnimation: function (fractionComplete, animationInfo) {
			...
			if (fractionComplete > 0)
				ctx.drawImage(
					...
					0,
					0,
					sourceCanvas.width * fractionComplete,
					sourceCanvas.height,
					0,
					0,
					(sourceCanvas.width * fractionComplete) /
						devicePixelBackingStoreRatio,
					sourceCanvas.height / devicePixelBackingStoreRatio
				);
